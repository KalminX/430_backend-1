...
def recipient_helper(recipient) -> dict:
    ...
    return {
        ...
        "id_no": recipient.get("id_no", ""),
        "phone_number": recipient.get("phone_number", ""),
        "gender": recipient.get("gender", ""),
        "address": recipient.get("address", ""),
        "house_hold_size": recipient.get("house_hold_size", 0),
        "house_hold_members": recipient.get("house_hold_members", []),
        "disability": recipient.get("disability", False),
    }
...
async def get_recipient_by_user_id(user_id: str):
    ...
    async for recipient in recipient_collection.find({"user_id": user_id}):
        ...
        if recipients:
            return recipients[0]  # Return the first recipient as a dict
        return None
