...
# Admin endpoint to fetch all donations
...
# Admin endpoint to get donation summary by food item (for pie chart)
@router.get("/admin/donations/fooditem-summary")
async def get_donations_fooditem_summary():
    pipeline = [
    ...
    async for item in donation_collection.aggregate(pipeline):
        summary.append({"food_item": item["_id"], "total": item["total"]})
    return summary
